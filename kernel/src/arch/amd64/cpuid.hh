...
static inline bool cpuid_is_emu() {
...
static inline bool cpuid_is_intel() {
	u32 eax = 0x00, ebx, ecx, edx;
	asm volatile ("cpuid" : "=c"(ecx), "=d"(edx), "=b"(ebx) : "a"(eax));
	return ebx == 'uneG' && edx == 'Ieni' && ecx == 'letn';
}

static inline bool cpuid_is_amd() {
	u32 eax = 0x00, ebx, ecx, edx;
	asm volatile ("cpuid" : "=c"(ecx), "=d"(edx), "=b"(ebx) : "a"(eax));
	return ebx == 'htuA' && edx == 'itne' && ecx == 'DMAc';
}

static inline u32 cpuid_max_leaf() {
	u32 eax = 0x00, ebx, ecx, edx;
	asm volatile ("cpuid" : "=a"(eax), "=c"(ecx), "=d"(edx), "=b"(ebx) : "a"(eax));
	return eax;
}

static inline bool cpuid_pcid_supported() {
	u32 eax = 0x01, ebx, ecx, edx;
	asm volatile ("cpuid" : "=c"(ecx), "=d"(edx), "=b"(ebx) : "a"(eax));
	return ecx & (1 << 17);
}

static inline u32 cpuid_x2apic_id() {
...
static inline bool cpuid_x2apic_supported() {
	u32 eax = 0x01, ebx, ecx, edx;
	asm volatile ("cpuid" : "=c"(ecx), "=d"(edx), "=b"(ebx) : "a"(eax));
	return ecx & (1 << 21);
}

static inline u32 cpuid_xapic_id() {
	u32 eax = 0x01, ebx, ecx, edx;
	asm volatile ("cpuid" : "=c"(ecx), "=d"(edx), "=b"(ebx) : "a"(eax));
	return ebx >> 24;
}
